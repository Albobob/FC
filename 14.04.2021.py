...
wb = load_workbook(filename='SMP.xlsx')
...
for i in range(len(nz)):
    ...
    p.add_run(f' Заболеваемость {name} в {district} в {last_year}г. составила {district_value} на 100 тыс. населения.\n')
    p.add_run(rs)
    ...
    for i in dict.general:
        ...
        if int(vl) == int(i):
            ...
            if one_rg_vl == 0:
                ...
                p.add_run(f'показателя по Российской Федерации ({one_rf_vl}){probel}{pt_l_one}{probel}{ttp_value_one}{probel}{tp_l_one}.'
                    f'Показатель по {district} составил {one_ds_vl} на 100 тыс. населения. \n')
                p.add_run(rs)
